# KB-002: Deploy schema objects (`deploy.sh`) — the routine deploy
...
## When to use
- After bootstrap ([KB-001](kb-001-db-bootstrap-new-environment.md)) to load the objects the
  first time.
...
| `relation/schema "…" does not exist` on first deploy | Environment was never bootstrapped | Run [KB-001](kb-001-db-bootstrap-new-environment.md) first |
...
## Related
- [KB-006: Apply-smoke & object tests](kb-006-db-apply-smoke-and-tests.md) — run this before merging DDL.
- [KB-003: Clean & redeploy](kb-003-db-clean-and-redeploy-schema.md) — when a broken state should be rebuilt without a full drop.
