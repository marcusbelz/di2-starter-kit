# KB-003: Bootstrap a new database environment (`create.sh`)
...
## Prerequisites
- A **running PostgreSQL server** the env's coordinates point to — `create.sh` creates the
  database *inside* an existing server, it does not start one. For `local` that is typically the
  Docker container from [KB-002](kb-002-local-postgres-docker-container.md); non-local envs run
  their own server (see `.claude/rules/deploy-infra.md`).
...
| `DB_FW_PASSWORD` | you choose | Set on the schema-owner role; needed again on **every deploy** ([KB-004](kb-004-db-deploy-schema-objects.md)). |
...
- **GitHub Actions:** configure them as **GitHub Environment secrets** on the target environment —
  see the secrets table in [KB-007](kb-007-github-actions-db-deployment-setup.md).
...
| Symptom | Cause | Fix |
|---------|-------|-----|
| `Error: database or roles for env '<env>' already exist` (exit 3 hint) | Bootstrap is **drop-and-recreate**; roles are cluster-global and survive `DROP DATABASE` | `bash db/scripts/drop.sh <env>` first, then re-run `create.sh` — see [KB-009](kb-009-db-drop-environment.md) |
...
| Connection refused / timeout in preflight | Wrong `DB_HOST`/`DB_PORT` in `<env>.env`, or the server is not running/reachable | Local: start the container ([KB-002](kb-002-local-postgres-docker-container.md)); otherwise verify the `.env` coordinates and firewall/tunnel |

## Related
- [KB-002: Local PostgreSQL Docker container](kb-002-local-postgres-docker-container.md) — the step before: provide the server.
- [KB-004: Deploy schema objects](kb-004-db-deploy-schema-objects.md) — the next step after bootstrap.
- [KB-009: Drop an environment](kb-009-db-drop-environment.md) — the inverse operation.
