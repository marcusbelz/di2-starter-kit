...
- [KB-007: Drop a database environment (`drop.sh`)](kb-007-db-drop-environment.md) — full teardown incl. the cluster-wide parameter-grant pitfall.
- [KB-008: How rules and skills are loaded (auto-load vs. on-demand)](kb-008-how-rules-and-skills-are-loaded.md) — why rules apply everywhere without being invoked, why skills still say "Read rules/X.md" (freshness + focus), and what that means for pruning and rule edits.
- [KB-009: Skills vs. agents — and how subagent dispatch works](kb-009-skills-vs-agents-subagent-dispatch.md) — how `.claude/agents/*.md` differ from skills, the built-in generic agents, and how the model picks a subagent via the frontmatter `description`.
